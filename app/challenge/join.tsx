...
import { getChallengeByCode, joinChallenge } from '../../src/lib/challengeService';
...
export default function JoinChallengeScreen() {
  ...
  const handleJoin = async () => {
    ...
    const challengeResult = await getChallengeByCode(code);
    ...
    if (!challengeResult.challenge) {
      setSubmitting(false);
      setSubmitError('No challenge found for that code — double check it and try again.');
      return;
    }
    ...
    const joinResult = await joinChallenge(challenge.id, joinerId);
    setSubmitting(false);
    if (!joinResult.ok) {
      setSubmitError("Couldn't join the challenge — check your connection and try again.");
      return;
    }
  ...
  return (
    <GradientScreen>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <Text style={styles.subtitle}>Enter the code a friend shared with you.</Text>
